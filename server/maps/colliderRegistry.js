...
function _load(mapId) {
  const rel = String(mapId || "").replace(/\\+/g, "/")
  const parts = rel.split("/")
  if (parts.length < 2) return { mapId, version: 0, bounds: null, solids: [] }

  const area = parts[0]
  const mapName = parts[1]

  // New convention: colliders/<area>/<map>/<map>.json
  const file = path.join(COLLIDERS_DIR, area, mapName, mapName + ".json")
  ...
  const shapes = Array.isArray(raw.solids)
    ? raw.solids
    : (Array.isArray(raw.props) ? raw.props : [])

  out.solids = shapes.map(_sanitizeSolid).filter(Boolean)
  return out
}
